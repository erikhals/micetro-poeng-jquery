(function () {
  ...
  var csRef = firebase.database().ref("shows/show/currentScene");
  var currentScene;
  ...
  $('#scenePage').on("create", e => {
    ...
    ref.once("value").then(snapshot => {
      ...
      snapshot.forEach(childSnap => {
        ...
        if (childval == true){
          $('#checkbox'+childSnap.key).prop('disabled', false);
        }
      });
      for (i=1; i<14; i++) {
  ...
  $("#btnNewShow").on('click', e => {
    ...
    for (var i=1;i<14;i++){
      $('#checkbox'+i).prop('disabled', false);
      $('#elcheckbox'+i).prop('disabled', false);
    };
  ...
  $("#btnEliminate").on('click', event => {
    ...
    playerListRef.once('value', function(snapshot){
      snapshot.forEach(function(playerSnapshot){
        ...
        $('#checkbox'+key).prop('disabled', false);
      });
      $('#btnElimination').hide();
